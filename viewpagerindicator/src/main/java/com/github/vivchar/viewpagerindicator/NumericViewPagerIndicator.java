 ...
 */
public class NumericViewPagerIndicator extends LinearLayoutCompat {
    ...
    @NonNull
    private final List<TextView> mIndexImages = new ArrayList<>();
    @Nullable
    private ViewPager.OnPageChangeListener mListener;

    public NumericViewPagerIndicator(@NonNull final Context context) {
        this(context, null);
    }

    public NumericViewPagerIndicator(@NonNull final Context context, @Nullable final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NumericViewPagerIndicator(@NonNull final Context context, @Nullable final AttributeSet attrs, final int defStyleAttr) {
    ...
    private void setSelectedIndex(final int selectedIndex) {
        ...
        final TextView unselectedView = mIndexImages.get(mSelectedIndex);
        unselectedView.animate().scaleX(NO_SCALE).scaleY(NO_SCALE).setDuration(300).start();
        unselectedView.setTextColor(mItemColor);

        final TextView selectedView = mIndexImages.get(selectedIndex);
        selectedView.animate().scaleX(mItemScale).scaleY(mItemScale).setDuration(300).start();
        selectedView.setTextColor(mItemSelectedColor);
    ...
    private FrameLayout createBoxedItem(final int position) {
        ...
        final TextView item = createItem(position);
        box.addView(item);
        mIndexImages.add(item);

        final LayoutParams boxParams = new LayoutParams(
    ...
    private TextView createItem(int position) {
        final TextView index = new TextView(getContext());
